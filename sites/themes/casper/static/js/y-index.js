$(document).ready(function () {
  ...
  // 技术支持
  // var apiMessageList = $(".messageApi .item");
  // var apiMailList = $(".mailApi .item");
  // var apiMessageContent = $(".messageContent .item");
  // var apiMailContent = $(".mailContent .item");
  ...
  var formTimer = 0;
  var signTimer = 0;
  var radomCodeKey = null;

  // 注册表单居中
  var signLeft = (Width - signIn.width()) / 2;
  signIn.css("left", signLeft + "px");

  //定义函数
  function slide() {
    applyBlock.stop().animate({
      marginTop: "0"
    }, 600);
  }
  ...
  function noteqie() {
    noteTimer = setInterval(function () {
      if (noteNum >= signLi.length - 1) {
        ...
        noteNum++;
        productBox.stop().animate({
          marginLeft: -200 * noteNum + "px"
        }, 300);
        descBox.stop().animate({
          marginLeft: -300 * noteNum + "px"
        }, 300);
      }
  ...
  function exampleSwitch(title, content, mark, desc, suit) {
    title.on("click", function () {
      ...
      title.eq(switchNum).css({
        color: "#26c3f2",
        backgroundColor: "#fff"
      });
      content.stop().animate({
        marginLeft: -1080 * switchNum + "px"
      }, 300);
  ...
  function autoSwitch(title, content, mark, desc, suit, sub) {
    timer = setInterval(function () {
      if (switchNum >= title.length - 1) {
        ...
        content.stop().animate({
          marginLeft: -moveDistance * switchNum + "px"
        }, 300);
      }
      for (var i = 0; i < title.length; i++) {
        title.eq(i).css({
  ...
  // 技术支持
  // for(var i = 0; i < apiMessageList.length; i++) {
  //   console.log(apiMessageList);
  //   apiMessageList.eq(i).css("color", "#000");
  //   apiMessageList.eq(i).click(
  //     function(){
  //       console.log(11);
  //       $(this).css({"color": "blue"});
  //     }
  ...
  //注册--点击刷新随机码
$(".refresh_code").unbind('click').click(function (e) {
  e.preventDefault();
  zcCode();
})
...
function zcCode() {
  $.ajax({
    url: "https://wx.xrewin.com/open/captcha",
    type: "get",
    processData: false,
    contentType: false,
    success: function (data) {
...
$(".sign").unbind('click').click(function (e) {
  // e.preventDefault();
  $(".signIn").css("display", "block"); //显示注册页面
  zcCode();
  document.getElementById("zc").reset();
 $("#zc").find('input').css("border-color", "#18a689");
 $(".mask-layer").css("display", "block");
});

// 随机码变化
$("#zccheckCode").on("input",function(e){
  //获取input输入的值
  if(e.delegateTarget.value) {
    $('.signIn_code_button').css('border', '1px solid #18a689');
    $('.signIn_code_button').css('backgroundColor', '#18a689');
    $('.signIn_code_button').css('color', '#fff');
    $('.signIn_code_button').css('pointer-events', 'auto');
  } else {
    $('.signIn_code_button').css('border', '1px solid #ccc');
    $('.signIn_code_button').css('backgroundColor', '#ccc');
    $('.signIn_code_button').css('color', '#fff');
    $('.signIn_code_button').css('pointer-events', 'none');
  }
});
// 注册--获取短信验证码
$(".signIn_code_button").unbind('click').click(function (e) {
  e.preventDefault();
  var data = {
    key: radomCodeKey,
    phone: $("#acmobileCode").val(),
    captcha: $("#zccheckCode").val(),
  }
  var jsonText = JSON.stringify(data);
  if ($("#zccheckCode").val() && $("#zccompany").val() && $("#acmobileCode").val()) {
    $.ajax({
      url: "https://wx.xrewin.com/open/captcha",
      type: "post",
      data: jsonText,
      ...
      success: function (data) {
        if (data.message === 'ok') {
          ...
          signTimer = setInterval(function () {
            if (times > 0) {
              $(".signIn_code_button").html(times + "s");
              $(".signIn_code_button").attr("disabled", "disabled");
              times--;
            } else {
              clearInterval(signTimer);
              $(".signIn_code_button").removeAttr("disabled");
              $(".signIn_code_button").html("点击获取验证码");
            }
          }, 1000);
        } else {
          alert('随机码错误');
  ...
  // $(".container").css("display", "block"); // 显示滑动验证页面
});

// 注册--点击注册按钮提交表单
$(".signOrder").unbind('click').click(function (e) {
  e.preventDefault();
  if (!cmd($("#zcForm"))) {
    alert('请将表单填写完整');
  }
  var data = {
    "company_name": $("#zccompany").val(),
    "phone": $("#acmobileCode").val(),
    "code": $("#zcsms").val(),
    "Pwd1": $("#zcpassword").val(),
    "Pwd2": $("#zcaffirm_password").val(),
    "Email": $("#zcaddr").val()
  }
  var jsonText = JSON.stringify(data);
  $.ajax({
    url: "https://wx.xrewin.com/open/manager",
    type: "post",
    data: jsonText,
    contentType: "application/json; charset=utf-8",
    dataType: "json",
    success: function (data) {
      if (data.message === 'ok') {
        ...
        $(".sign_success").css("display", "block"); //显示注册成功页面
        ...
        signTimer = setInterval(function () {
          if (times > 0) {
            ...
            clearInterval(signTimer);
            $(".sign_success").css("display", "none");
            window.open('https://wx.xrewin.com/login', '_self');
          }
        }, 1000);
      } else {
        alert(data.message);
...
// 注册--协议
$(".weui-check").unbind('click').click(function (e) {
  console.log($('.weui-check').is(':checked'));
  if (!$('.weui-check').is(':checked')) {
    $('.signOrder').css('backgroundColor', '#ccc');
    $('.signOrder').css('pointer-events', 'none');
  } else {
    $('.signOrder').css('backgroundColor', '#18a689');
    $('.signOrder').css('pointer-events', 'auto');
  }
})
$(".zcicon").mouseenter(function () {
  $(this).find('.iconIntruct').css("display", "block");
});
$(".zcicon").mouseleave(function () {
  $(this).find('.iconIntruct').css("display", "none");
});

  //手机正则
  $("#acmobileCode").unbind('blur').blur(function (e) {
    var t = /^1[345789]\d{9}$/;
    if (!t.test($(this).val())) {
      alert('手机格式错误');
  ...
  //邮件正则
  $("#zcaddr").unbind('blur').blur(function (e) {
    var re = /(^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+)|(^$)/;
    if (!re.test($(this).val())) {
      alert('邮件格式错误');
      $(this).css("border-color", "red");
  ...
  //注册页密码验证
  $("#zcpassword").unbind('blur').blur(function (e) {
